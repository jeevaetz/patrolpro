 ...
 * @author ira
 */
public class LoadEmployeeReportUsageBreakdownServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArrayList<MobileReportEmployeeGraphData> mobileData = mobileService.getMobileReportEmployeeGraphDataSummary(dbFormat.format(startCal.getTime()), dbFormat.format(endCal.getTime()), interval, selectedClientIdsInt);
            
            LinkedHashMap<String, JFlotPieSlice> currentLineHash = new LinkedHashMap<String, JFlotPieSlice>();
            for (int d = 0; d < mobileData.size(); d++) {
                MobileReportEmployeeGraphData currData = mobileData.get(d);

                if (!currentLineHash.containsKey(currData.getEmployeeName())) {
                    currentLineHash.put(currData.getEmployeeName(), new JFlotPieSlice());
                }
                JFlotPieSlice currPieSlice = currentLineHash.get(currData.getEmployeeName());

                currPieSlice.setData(currPieSlice.getData() + currData.getNumberFilledOut());
                currPieSlice.setLabel(currData.getEmployeeName());
            }
